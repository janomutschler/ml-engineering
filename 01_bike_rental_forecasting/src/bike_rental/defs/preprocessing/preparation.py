...
def prepare_operational_rentals(
    ...
    reason_masks = {
        ...
        "missing or negative location_id": (
            rentals["location_id"].isna() | (rentals["location_id"] < 0)
        ),
    }
...
def prepare_weather(weather: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    prepared_weather["datetime"] = pd.to_datetime(
        prepared_weather["datetime"],
        errors="coerce",
    )
...
def prepare_holidays(
    ...
    prepared_holidays["date"] = pd.to_datetime(
        prepared_holidays["date"],
        errors="coerce",
    ).dt.date
