...
def prepared_operational_rentals(raw_booked_rentals, raw_direct_pickups):
	"""Materialize all validated structured operational rental records and quarantine invalid rows."""
...
def prepared_weather(raw_weather):
	...
	missing_weather_values = {
		column: int(valid_weather[column].isna().sum())
		for column in [
			"conditions",
			"temperature_c",
	...
	return MaterializeResult(
		...
		metadata=build_dataframe_metadata(
			...
			extra_metadata={
				...
				"missing_values": missing_weather_values,
			},
		),
	)
